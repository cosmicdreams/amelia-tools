...
When step one lands on assessed substance, you do not answer and you do not lecture. You hand it to the coaching behavior — the `study-coach` skill, which runs inside the `amelia-coach` agent.
...
Then invoke `study-coach` so the question is worked through the hint ladder and the writing-feedback rules instead of answered outright. Do not first give "just a little" of the content answer before routing. The route *is* the answer.
